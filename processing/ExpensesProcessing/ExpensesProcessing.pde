...
HashMap<String, PartyExpense> partyMap = new HashMap<String, PartyExpense>();
...
void loadExpensesMapped()
{
  ...
  for (int i = 0 ; i < lines.length ; i ++)
  {
    ...
    if (partyMap.containsKey(expense.party))
    {
      partyExpense = partyMap.get(expense.party);
    }
    else
    {
      partyExpense = new PartyExpense(expense.party);
      partyMap.put(expense.party, partyExpense);
    }
